...
struct permiso{
	//pais izquierdo, pais derecho, costo
	long long int l,r,w;
};
...
int main() {
	...
	for(permiso el: tratados){
		//busca el minimo costo en el rango que cubre el tratado, y le suma el costo del tratado pues fue necesario usarlo para acceder a los elementos del rango
		c = st.query(el.l,el.r)+el.w;
		//si el costo para llegar al extremo derecho del tratado actual es mayor al costo calculado en esta iteracion, actualizalo
		if(st.query(el.r,el.r)>c){
			st.update(c,el.r);
		}
	}
 
	//pregunta al segment tree el costo de llegar a m
	long long int res = st.query(m,m);
	
	//si cuesta infinito, significa que es imposible llegar, si es cualquier otra cosa ese es el costo de llegar
